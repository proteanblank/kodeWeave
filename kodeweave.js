...
$(document).ready(function() {
  ...
  $("[data-action=check]").on("change", function() {
    ...
    closeRefs.setValue($("[data-action=library-code]").val() + "\n  </head>\n  <body>\n");
  });
      ...
      reader.onload = function(e) {
        ...
        $("[data-action=export-as-win-app]").on("click", function() {
          ...
          JSZipUtils.getBinaryContent("YourWinApp.zip", function(err, data) {
            ...
            var zip = new JSZip();
            var appName = zip.folder( $(".vprojectname").val().replace(/ /g, "-")  );
            appName.load(data);
            ...
            var content = zip.generate({type:"blob"});
            saveAs(content, $(".vprojectname").val().replace(/ /g, "-") + "-win.zip");
          });
        });
