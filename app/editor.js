...
if (window.location.hash) {
  function loadgist(gistid) {
    $.ajax({
      ...
      var settings   = gistdata.data.files["libraries.json"].content
  ...
  $("[data-action=save-gist]").click(function() {
    ...
    if ( !htmlEditor.getValue()) {
      htmlEditor.setValue("<!-- autogenerated to save online -->")
    }
    if ( !cssEditor.getValue()) {
      cssEditor.setValue("/* autogenerated to save online */")
    }
    if ( !jsEditor.getValue()) {
      jsEditor.setValue("// autogenerated to save online")
    }
    if ( !mdEditor.getValue()) {
      mdEditor.setValue("<!-- autogenerated to save online -->")
    }

    htmlVal = htmlEditor.getValue()
    cssVal  =  cssEditor.getValue()
    jsVal   =   jsEditor.getValue()
    mdVal   =   mdEditor.getValue()
    libraries = JSON.stringify(arr)

    data = {
      "description": $("[data-action=sitedesc]").val(),
      "public": true,
      "files": {
        "index.html"      : { "content": htmlVal },
        "index.css"       : { "content": cssVal },
        "index.js"        : { "content": jsVal },
        "README.md"       : { "content": mdVal },
        "libraries.json"  : { "content": libraries }
      }
    }
